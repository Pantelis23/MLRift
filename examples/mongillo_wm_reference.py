#!/usr/bin/env python3
"""Reference for examples/mongillo_wm.mlr — stage 9 + stimulus injection
into neurons 0..19 during steps 1000..1999, u_stp checkpoints after."""
...
stim_start = 1000
stim_end = 2000
stim_lo, stim_hi = 0, 20
base_lo, base_hi = 20, 40
...
def report_u(step, lo, hi, label):
    s = sum(u_stp[lo:hi])
    mean_ppm = trunc(s / (hi - lo) * 1000000.0)
    tag = "stim" if label == 0 else "base"
    print(f"# u_at_step={step} group={tag} mean_ppm={mean_ppm}")
...
for step in range(n_steps):
    ...
    stim_now = stim_amp if (step >= stim_start and step < stim_end) else 0.0
    ...
    if step in (1999, 4999, 7999, 9999):
        report_u(step, stim_lo, stim_hi, 0)
        report_u(step, base_lo, base_hi, 1)
