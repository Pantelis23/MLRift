#!/usr/bin/env python3
"""Reference for examples/stdp_100.mlr — pair-based STDP on E→E synapses
of the stage-7 E/I network. IEEE-754 f64 throughout."""
...
for step in range(n_steps):
    ...
    for syn in range(n_syn):
        ...
        if fired_this_step[src]:
            if is_inh:
                s_inh[tgt] = s_inh[tgt] + w
            else:
                s_exc[tgt] = s_exc[tgt] + w
...
for sc in range(n_syn):
    if not syn_is_inh[sc]:
        sum_w = sum_w + syn_w[sc]
        n_ee += 1
# Truncate toward zero to match KernRift's f64_to_int semantics.
sum_w_ppm_f = sum_w * 1000000.0
sum_w_ppm = int(sum_w_ppm_f) if sum_w_ppm_f >= 0 else -int(-sum_w_ppm_f)

print(f"# ltp_events={ltp_events}")
print(f"# ltd_events={ltd_events}")
print(f"# n_ee_synapses={n_ee}")
print(f"# sum_ee_weight_ppm={sum_w_ppm}")
