...
#                    cached under a stamp of (corpus realpath, corpus size,
#                    vocab size) and regenerated whenever any of those differ.
#   HF_BAR_1T/_FULL  measured on $DEFAULT_CORPUS at $BAR_VOCAB. If this run
#                    tests anything else they cannot decide a gate, so
...
if [ "$CORPUS_REAL" != "$(readlink -f "$DEFAULT_CORPUS")" ]; then
    ...
    BARS_APPLY=0
    BARS_WHY="vocab_size $VOCAB_SIZE != the bars' $BAR_VOCAB"
elif [ "$NTHREADS_FULL" != "$BAR_CORES" ]; then
    BARS_APPLY=0
...
# run costs more than it buys, and a same-path same-size edit is not a threat
# model this harness has.)
...
if [ -s "$REF_JSON" ] && [ -s "$REF_MIN" ] && [ -f "$REF_STAMP" ] \
   && [ "$(cat "$REF_STAMP")" = "$WANT_STAMP" ]; then
    ...
    "$PY" - "$CORPUS" "$REF_JSON" "$VOCAB_SIZE" <<'PYGEN' || { echo "FATAL: HF reference generation failed" >&2; exit 1; }
