#!/usr/bin/env python3
"""Reference for scale_10k_vec.mlr — same math, but the decay sweep
uses precomputed factors (f_e = dt/tau_e etc) rather than recomputing
dt/tau_e inside the loop. Output differs from scale_10k_reference.py
by ≤1 ULP per operation because of associative re-ordering of the
scalar ops; that's the whole point of the vec_f64_decay_inplace
signature."""
...
def vec_decay(buf, n, f):
    # 4-way unrolled to match kr helper's evaluation order exactly.
    i = 0
    n4 = n & ~3
    while i < n4:
...
sum_u = vec_sum(u_stp, N)
sum_x = vec_sum(x_stp, N)
